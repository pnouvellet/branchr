---
title: "Example"
author: "Pierre Nouvellet "
output:
  html_document:
    df_print: paged
  pdf_document: default
  word_document: default
---

```{r}

rm(list=ls(all.names=TRUE))
library(Hmisc)
library(branchr)

```

# Simulate

## Poisson


```{r}


s <- 1.    # number of initial case per cluster
t_max <- 5e3    # simulate for how many days
overdispersion <-  c(NA, 1, 0.1)
n_over <- length(overdispersion)

param.grid <- expand.grid(R = c(.5, 0.8, 0.9),  # reproduction number
                          rho = c(0.2,0.8),       # reporting
                          n = c(50, 100, 500), # number if independent importations
                          k = overdispersion ) # overdisperrsion



Nrep <- 100 # number of replications
n_param_set <- nrow(param.grid)

param.grid$R_med = NA
param.grid$R_low = NA
param.grid$R_high = NA
param.grid$R_cover = NA

param.grid$Imp_med = NA
param.grid$Imp_low = NA
param.grid$Imp_high = NA
param.grid$Imp_cover = NA


```

# simulate Poisson clusters

```{r}

Clusters <- list()
for(k in 1:n_param_set){
  print(k)
  Clusters[[paste0('Rkrhon = ',paste(param.grid[k,1:4],collapse = ' ; '))]] = list()
  for(i in 1:Nrep){
    if(is.na(param.grid$k[k])){
      temp <- simulate_poisson(R = param.grid$R[k],
                               n = param.grid$n[k], 
                               s = s, 
                               rho = param.grid$rho[k], 
                               t_max = t_max)
    }else if(param.grid$k[k] > 0){
      temp <- simulate_negbin(R = param.grid$R[k],
                              n = param.grid$n[k], 
                              s = s, 
                              rho = param.grid$rho[k], 
                              t_max = t_max,  
                              over = param.grid$k[k])
    }
    
    Clusters[[k]][[i]] <- list(Sizes = temp$true_size, # true list of outbreak sizes
                               Sizes = temp$reported_size, # observed list of outbreak sizes
                               y_obs = temp$observed_size) # size reported
    
  }
}

saveRDS(list(Clusters = Clusters, param.grid = param.grid), file = 'sim_grid.rds')

```

```{r, eval=FALSE}

a <- readRDS(file = 'sim_grid.rds')
Clusters <- a$Clusters
a$param.grid

```

# inference

```{r}

Full <- list()
for(k in 1:n_param_set){
  print(k)
  Rest <- Imp <-list(theta = rep(NA,Nrep), CIin = rep(NA,Nrep))
  
  if (is.na(param.grid$k[k])){
    overd <- NULL
  }else if(param.grid$k[k]>0){
    overd <- param.grid$k[k]
  }
  for(i in 1:Nrep){
    
    profile <- profile_likelihood(y_obs = Clusters[[k]][[i]]$y_obs,
                                  rho = param.grid$rho[k],
                                  accuracy = 0.01,
                                  max_R = 2,
                                  over = overd)
    
    # estimate R
    R_estimate <- theta_max_likelihood(theta = profile$theta,
                                       likelihood = profile$Likelihood)
    
    # estimate number of importations unseen
    imp_est <- import(y_obs = Clusters[[k]][[i]]$y_obs,
                     rho = param.grid$rho[k],
                     profile = profile,
                     threshold_z = 1e3,
                     threshold_import = 1e3,
                     over = overd)
    
    Rest$theta[i] <- R_estimate$theta_max_likelihood
    Rest$CIin[i] <- ((R_estimate$upper_theta >= param.grid$R[k]) &
                       (R_estimate$lower_theta <= param.grid$R[k]))
    
    Imp$theta[i] <- length(Clusters[[k]][[i]]$y_obs)+imp_est$theta_max_likelihood
    Imp$CIin[i] <- (( (length(Clusters[[k]][[i]]$y_obs)+imp_est$upper_theta) >= param.grid$n[k]) &
                      ( (length(Clusters[[k]][[i]]$y_obs)+imp_est$lower_theta) <= param.grid$n[k]))
    
  }
  Full[[k]] <- list(Rest = Rest, Imp = Imp)
  y <- Rest
  param.grid[k,5:8] <- c( quantile(y$theta,c(.5,.025,.975)) , sum(y$CIin)/Nrep)
  
  y <- Imp
  param.grid[k,9:12] <- c( quantile(y$theta,c(.5,.025,.975)) , sum(y$CIin)/Nrep)
  
}

saveRDS(list(Rest = Rest, Imp = Imp, Full = Full,
             Clusters = Clusters, param.grid = param.grid), 
        file = 'results_sim_grid.rds')


```


```{r, eval=FALSE}

aa <- readRDS('results_sim_grid.rds')

Rest = aa$Rest
Imp = aa$Imp
Full = aa$Full

Clusters = aa$Clusters
param.grid = aa$param.grid

```

## plot

```{r,  fig.height=4, fig.width= 4}

layout(matrix(c(1,3,2),nrow = 3,ncol = 1))

par(oma=c(3,3,0,0))
# plot R's
Hmisc::errbar(x = rep(1:(n_param_set/n_over), n_over)+
                rep(seq(-0.1,0.1, length.out = n_over), each = n_param_set/n_over),
              y = param.grid$R_med, 
              yminus = param.grid$R_low, 
              yplus = param.grid$R_high,
              bty = 'n',ylim = c(0,1),
              ylab = 'distribution of R estimates',
              xaxt = "n", xlab = "",
              col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over),
              errbar.col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over))

# axis(1, at = 1:(n_param_set/n_over), 
#      labels = paste0('R=',param.grid$R,'; rho=',param.grid$rho,
#                      '; n=',param.grid$n)[1:(n_param_set/n_over)], 
#      las=2, cex.axis = 0.8)
axis(1, at = 1:(n_param_set/n_over), 
     labels = FALSE, 
     las=2, cex.axis = 0.8)

lines(1:(n_param_set/n_over), param.grid$R[1:(n_param_set/n_over)],
      pch = 16, type = 'p', col = 'red3')

legend('bottomright',
       legend = c('Simulated values','Poisson','NB (over = 1))','NB (over = 0.1)'),
       col = c('red3','blue3','green4','orange3'), bty = 'n',pch = 16)

# coverage
temp <- Hmisc::binconf(x = param.grid$R_cover*Nrep, n = Nrep, method = 'exact')
Hmisc::errbar(rep(1:(n_param_set/n_over), n_over)+
                rep(seq(-0.1,0.1, length.out = n_over), each = n_param_set/n_over), 
              y = temp[,1],
              yminus = temp[,2], 
              yplus = temp[,3],
              bty = 'n',ylim = c(0,1),
              ylab = 'Proportion of 95%CI including true R',
              xaxt = "n", xlab = "",
              col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over),
              errbar.col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over))

axis(1, at = 1:(n_param_set/n_over), 
     labels = paste0('R=',param.grid$R,'; rho=',param.grid$rho,
                     '; n=',param.grid$n)[1:(n_param_set/n_over)], 
     las=2, cex.axis = 0.8)
abline(h = 0.95, lty = 2, col = 'red3')

legend('bottomright',
       legend = c('Poisson','NB (over = 1))','NB (over = 0.1)'),
       col = c('blue3','green4','orange3'), bty = 'n',pch = 16)


# bias
fct1 <- function(x,i){
  a <- quantile(x$Rest$theta - param.grid$R[i],c(0.5,.025,0.975)) / param.grid$R[i]
  b <- quantile(x$Imp$theta - param.grid$n[i],c(0.5,.025,0.975)) / param.grid$n[i]
  return(c(a,b))
}
param.grid$Rbias <- param.grid$RbiasLow <- param.grid$RbiasHigh <- NA
param.grid$Impbias <- param.grid$ImpbiasLow <- param.grid$ImpbiasHigh <- NA
for(i in 1:n_param_set){
  param.grid[i,c('Rbias','RbiasLow','RbiasHigh',
                 'Impbias','ImpbiasLow','ImpbiasHigh')] <- fct1(x = Full[[i]], i = i)
}

Hmisc::errbar(rep(1:(n_param_set/n_over), n_over)+
                rep(seq(-0.1,0.1, length.out = n_over), each = n_param_set/n_over), 
              y = param.grid$Rbias,
              yminus = param.grid$RbiasLow, 
              yplus = param.grid$RbiasHigh,
              bty = 'n',ylim = c(-1,1),
              ylab = 'Bias in R',
              xaxt = "n", xlab = "",
              col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over),
              errbar.col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over))

# axis(1, at = 1:(n_param_set/n_over), 
#      labels = paste0('R=',param.grid$R,'; rho=',param.grid$rho,
#                      '; n=',param.grid$n)[1:(n_param_set/n_over)], 
#      las=2, cex.axis = 0.8)
axis(1, at = 1:(n_param_set/n_over), 
     labels = FALSE, 
     las=2, cex.axis = 0.8)
abline(h = 0, lty = 2, col = 'red3')

legend('bottomright',
       legend = c('Poisson','NB (over = 1))','NB (over = 0.1)'),
       col = c('blue3','green4','orange3'), bty = 'n',pch = 16)
```

```{r, fig.height=4, fig.width= 4}

layout(matrix(c(1,3,2),nrow = 3,ncol = 1))

#plot importations
Hmisc::errbar(x = rep(1:(n_param_set/n_over), n_over)+
                rep(seq(-0.1,0.1, length.out = n_over), each = n_param_set/n_over),
              y = param.grid$Imp_med, 
              yminus = param.grid$Imp_low, yplus = param.grid$Imp_high,
              bty = 'n',ylim = c(0,max(param.grid$n*1.2)),
              ylab = 'distribution of importation estimates',
              xaxt = "n", xlab = "",
              col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over),
              errbar.col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over))

# axis(1, at = 1:(n_param_set/n_over),
#      labels = paste0('R=',param.grid$R,'; rho=',param.grid$rho,
#                      '; n=',param.grid$n)[1:(n_param_set/n_over)], 
#      las=1, cex.axis = 0.8)
axis(1, at = 1:(n_param_set/n_over), 
     labels = FALSE, 
     las=2, cex.axis = 0.8)
lines(1:(n_param_set/2), param.grid$n[1:(n_param_set/2)],
      pch = 16, type = 'p', col = 'red3')

legend('bottomright',
       legend = c('Simulated values','Poisson','NB (over = 1))','NB (over = 0.1)'),
       col = c('red3','blue3','green4','orange3'), bty = 'n',pch = 16)

# coverage
temp <- Hmisc::binconf(x = param.grid$Imp_cover*Nrep, n = Nrep, method = 'exact')
Hmisc::errbar(rep(1:(n_param_set/n_over), n_over)+
                rep(seq(-0.1,0.1, length.out = n_over), each = n_param_set/n_over), 
              y = temp[,1],
              yminus = temp[,2], 
              yplus = temp[,3],
              bty = 'n',ylim = c(0,1),
              ylab = 'Proportion of 95%CI including true importations',
              xaxt = "n", xlab = "",
              col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over),
              errbar.col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over))

axis(1, at = 1:(n_param_set/n_over),
     labels = paste0('R=',param.grid$R,'; rho=',param.grid$rho,
                     '; n=',param.grid$n)[1:(n_param_set/n_over)],
     las=2, cex.axis = 0.8)
abline(h = 0.95, lty = 2, col = 'red3')

legend('bottomright',
       legend = c('Poisson','NB (over = 1))','NB (over = 0.1)'),
       col = c('blue3','green4','orange3'), bty = 'n',pch = 16)


# bias
Hmisc::errbar(rep(1:(n_param_set/n_over), n_over)+
                rep(seq(-0.1,0.1, length.out = n_over), each = n_param_set/n_over), 
              y = param.grid$Impbias,
              yminus = param.grid$ImpbiasLow, 
              yplus = param.grid$ImpbiasHigh,
              bty = 'n',ylim = c(-1,1),
              ylab = 'Bias in importations',
              xaxt = "n", xlab = "",
              col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over),
              errbar.col = rep(c('blue3','green4','orange3'), each = n_param_set/n_over))

# axis(1, at = 1:(n_param_set/n_over), 
#      labels = paste0('R=',param.grid$R,'; rho=',param.grid$rho,
#                      '; n=',param.grid$n)[1:(n_param_set/n_over)], 
#      las=2, cex.axis = 0.8)
axis(1, at = 1:(n_param_set/n_over), 
     labels = FALSE, 
     las=2, cex.axis = 0.8)
abline(h = 0, lty = 2, col = 'red3')

legend('bottomright',
       legend = c('Poisson','NB (over = 1))','NB (over = 0.1)'),
       col = c('blue3','green4','orange3'), bty = 'n',pch = 16)
```
#. checks / debug

<!-- ```{r} -->

<!-- k=54 -->
<!-- i=1 -->


<!-- y_obs = Clusters[[k]][[i]]$y_obs -->
<!-- y_true = Clusters[[k]][[i]]$Sizes -->
<!-- rho = param.grid$rho[k] -->
<!-- over = param.grid$k[k] -->
<!-- n <- param.grid$n[k] -->
<!-- R <- param.grid$R[k] -->

<!-- a <- simulate_negbin(R, n = 1e4, s = 1, rho = rho, t_max = 1e3, over = over) -->
<!-- y_obs <- a$observed_size -->
<!-- y_true <- a$true_size -->
<!-- y_trueobs <- a$reported_size -->

<!-- ``` -->

<!-- ```{r} -->

<!-- h1 <- hist(y_true,breaks = seq(0,1e5)) -->
<!-- ``` -->

<!-- ```{r} -->


<!-- plot(log10(h1$breaks[-1]),log10(h1$density), ylim = c(-5,0)) -->

<!-- f1 <- f_z_NB(z = h1$breaks[-1], s = 1, R = R, over = over) -->
<!-- lines(log10(h1$breaks[-1]),log10(f1)) -->

<!-- f2 <- f_z_NB(z = h1$breaks[-1], s = 1, R = .8, over = over) -->
<!-- lines(log10(h1$breaks[-1]),log10(f2),col = 'red') -->


<!-- # naturascale -->

<!-- plot(log10(h1$breaks[-1]),(h1$density), ylim = c(0,1)) -->
<!-- lines(log10(h1$breaks[-1]),(f1),col = 'black') -->
<!-- lines(log10(h1$breaks[-1]),(f2),col = 'red') -->

<!-- ``` -->

<!-- ```{r} -->

<!-- threshold <- max(c(max(y_obs)+qnbinom(0.999, max(y_obs),rho,lower.tail=TRUE)+1), -->
<!--                    1e3) -->

<!-- Y <- matrix(y_true,nrow = length(y_true), ncol = threshold, byrow = FALSE) -->
<!-- # Plausible outbreak sizes -->
<!-- z <- matrix(seq(1,threshold), nrow = threshold, ncol = 1) -->

<!-- # likelihood of observing y given z,rho -->
<!-- g <- dbinom(Y,matrix(z, nrow = nrow(Y), ncol = threshold, byrow = TRUE),prob = 1) -->
<!-- # likelihood of oberving 0 given z, rho -->
<!-- g0 <- dbinom(0,matrix(z, nrow = 1, ncol = threshold, byrow = TRUE),prob = 1) -->

<!-- Rs <- seq(0.6,0.95,by=.01) -->
<!-- L0 <- rep(NA,length(Rs)) -->
<!-- for(i in 1:length(Rs)){ -->
<!--   f <- f_z_NB(z = z, s = 1, R = Rs[i], over = over) #*R_eff$P_extinction #all R_eff are  -->
<!--   L0[i] <- sum(log((g %*% f))) -->
<!-- } -->

<!-- g0 <- dbinom(0,matrix(z, nrow = 1, ncol = threshold, byrow = TRUE),prob = rho) -->
<!-- sum(y_trueobs[y_true==1]==0)/sum(y_true==1) -->
<!-- sum(y_trueobs[y_true==2]==0)/sum(y_true==2) -->

<!-- f1 <- f_z_NB(z = z, s = 1, R = 0.9, over = over) #*R_eff$P_extinction #all R_eff are  -->
<!-- aa=(log((g %*% f1))) -->

<!-- f2 <- f_z_NB(z = z, s = 1, R = 0.8, over = over) #*R_eff$P_extinction #all R_eff are  -->
<!-- bb=(log((g %*% f2))) -->

<!-- sum(aa) -->
<!-- sum(bb) -->

<!-- # with obsered, including 0 -->
<!-- # likelihood of observing y given z,rho -->
<!-- z <- matrix(seq(0,threshold), nrow = threshold+1, ncol = 1) -->
<!-- Y <- matrix(y_trueobs,nrow = length(y_trueobs), ncol = threshold+1, byrow = FALSE) -->
<!-- g <- dbinom(Y,matrix(z, nrow = nrow(Y), ncol = threshold+1, byrow = TRUE),prob = rho) -->
<!-- # likelihood of oberving 0 given z, rho -->
<!-- g0 <- dbinom(0,matrix(z, nrow = 1, ncol = threshold+1, byrow = TRUE),prob = rho) -->

<!-- Rs <- seq(0.6,0.95,by=.01) -->
<!-- Li <-  rep(NA,length(Rs)) -->
<!-- for(i in 1:length(Rs)){ -->
<!--   f <- f_z_NB(z = z, s = 1, R = Rs[i], over = over) #*R_eff$P_extinction #all R_eff are  -->
<!--   Li[i]<-sum(log((g %*% f))) -->
<!-- } -->




<!-- # with obsered, excluding 0 -->
<!-- # likelihood of observing y given z,rho -->
<!-- z <- matrix(seq(1,threshold), nrow = threshold, ncol = 1) -->

<!-- Y <- matrix(y_obs,nrow = length(y_obs), ncol = threshold, byrow = FALSE) -->
<!-- g <- dbinom(Y,matrix(z, nrow = nrow(Y), ncol = threshold, byrow = TRUE),prob = rho) -->
<!-- # likelihood of oberving 0 given z, rho -->
<!-- g0 <- dbinom(0,matrix(z, nrow = 1, ncol = threshold, byrow = TRUE),prob = rho) -->

<!-- Rs <- seq(0.6,0.95,by=.01) -->
<!-- L <- L2 <- rep(NA,length(Rs)) -->
<!-- for(i in 1:length(Rs)){ -->
<!--   f <- f_z_NB(z = z, s = 1, R = Rs[i], over = over) #*R_eff$P_extinction #all R_eff are  -->
<!--   L[i]<-sum(log((g %*% f))) -->
<!--   L2[i]<-sum(log((g %*% f))) - nrow(g)*log(1-g0 %*% f) -->
<!-- } -->

<!-- ``` -->

<!-- ```{r} -->

<!-- plot(Rs,L0-max(L0)) -->
<!-- theta_max_likelihood(theta = Rs, likelihood = L0) -->

<!-- lines(Rs,L-max(L)) -->
<!-- theta_max_likelihood(theta = Rs, likelihood = L) -->
<!-- lines(Rs,Li-max(Li),type='p',pch=16,col='green4') -->
<!-- theta_max_likelihood(theta = Rs, likelihood = Li) -->
<!-- lines(Rs,L2-max(L2),col='red') -->
<!-- theta_max_likelihood(theta = Rs, likelihood = L2) -->

<!-- ``` -->


<!-- ```{r} -->
<!-- f_z_NB2 <- function(z,s,R,over){ -->
<!--   z <- z+1 -->
<!--   logf<-lgamma(z*over+z-1)-lgamma(z*over)-lgamma(z+1) + (z-1)*log(R/over) - (z*over+z-1)*log(R/over+1) -->
<!--   f<-exp(logf) -->

<!--   # logf<-(z-s-1)*log( s*z)+(z-s)*log(R)+ (-z*R) - lgamma(z-s +1) -->
<!--   # f<-exp(logf) -->

<!--   # plot(Re(Psize)) -->
<!--   # sum(Psize,na.rm=TRUE) -->

<!--   return(proba_z=f) -->

<!-- } -->

<!-- ``` -->
